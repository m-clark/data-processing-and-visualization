...
We can also describe two primary goals of modeling:
...
While these are sometimes competing goals, it is definitely not the case that they are mutually exclusive. For example, a fully interpretable model (statistically speaking) with no predictive capability is fairly useless in practical terms.

## Linear models

By far most models you see in published reports are linear models, even when they incorporate nonlinear relationships.  For example, in the following, we can add a quadratic term or an interaction, yet the model is still linear in the parameters.  All of the following are standard linear regression models.
...
In two of the models above, `x` has a potentially nonlinear relationship with `y`, either by varying its relationship depending on values of z (the first case) or itself (the second). In general, the manner in which nonlinear relationships may be explored in linear models is quite flexible.
...
In a basic statistics course, most are introduced to ordinary least squares as a means to estimate the coefficients for a linear regression model.  We are seeking to come up with estimates of the coefficients that minimzed the (squared) difference between the target value and the estimated value based on the parameter estimates.  We can see this clearly with some simple conceptual code.
...
In some rare cases, a known approach is available and we do not have to search for the best estimates, but simply have to perform the correct steps that will result in them.  For example the following matrix operations will produce the best estimates for linear regression, which also happen to be the maximum likelihood estimates.
...
Most of the time we don't have such luxury, or even if we did, the computations might be too great for the size of our data.  In practice we use  *optimization* methods to search for the best estimates. Some are very general, some are better for certain types of modeling problems. 

Most statistical modeling techniques use *maximum likelihood* in some form or fashion, including Bayesian approaches. In this case, instead of minimizing the loss, we use an approach to maximize the probability of the observations of the target variable given the estimates of the parameters of the model (e.g. the coefficients in a regression).  A basic linear model may use ordinary least squares, others generalized estimating equations, but far and away the most common approach in statistical modeling is maximum likelihood, so you would do well to understand the basics.  Again, some simple conceptual code can help us. In the following I fix the estimated variance, but in practice we'd need to estimate that parameter as well.  As probabilities are typically very small, we work with them on the log scale.
...
Whatever the goal, to maximize the likelihood of the target data or to minimize a loss function, an optimizing function will typically be used to find the estimates that reach our goal. 


In the more predictive models previously discussed
...
Note the column of ones in the model matrix `X`.  This represents our intercept, but may not mean much to you unless you understant [matrix multiplication](http://matrixmultiplication.xyz/).  The other columns are just as they are in the data.  Note also that the missing values have been removed.
