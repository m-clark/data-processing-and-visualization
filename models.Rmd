...
In R, we specify a linear model as follows.  The target variable is first, followed by the predictor variables, separated by a tilde.
...
We see that in our third guess, we reduce the sum of the squared errors, i.e. the *loss*, quite a bit. However, we can also see that they are not the best we could have done.  With more data, our estimated coefficients would get closer to true values.
