...
R provides the means to read and store compressed data types.  While there are a variety of ways to do so <span class="" style = "">save</span> and <span class="func" style = "">save.image</span> are probably the most common. To save a one or more objects we can use save as follows:
...
The <span class="func" style = "">save.image</span> function works just like <span class="func" style = "">save</span>, but saves all objects currently in your working environment.  You would still just use load to load the objects back into your working environment[^savews].
...
```{r data-load}
data(mcycle, package = 'MASS') # loads data
head(mcycle)
```
