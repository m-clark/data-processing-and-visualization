...
ga_rf_results = rand_forest(mode = 'regression', mtry = 2, trees = 1000) %>%
  set_engine(engine = "ranger", importance = 'impurity_corrected') %>% 
  fit(
    avg_sentiment_polarity ~ type + reviews + installs + size_in_MB + category,
    google_for_mod
  )

ga_rf_results
sort(ga_rf_results$fit$variable.importance)
ranger::importance_pvalues(ga_rf_results$fit)
ranger::importance_pvalues(
  ga_rf_results$fit,
  method = 'altmann',
  formula = avg_sentiment_polarity ~ type + reviews + installs + size_in_MB + category,
  data = google_for_mod
)
```
