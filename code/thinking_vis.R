...
sw_height %>% 
  ...
  geom_bar(aes(fill=sex), color='black', stat='identity', width=.5) +
  scale_fill_manual(values=c('red1', 'green')) +
...
sw2 = starwars %>% 
  ...
  select(name, mass, height, birth_year, sex, homeworld, homeworld2) %>% ungroup
...
ggplotly(width = 600, height = 400) %>%
  layout(
    title = list(text = 'Star Wars Characters',
                 x = 0.1,
                 font = list(size = 16)),
    font  = list(family = 'Roboto'),
    xaxis = list(title  = 'Mass in kg', titlefont = list(size = 12), x = 0),  # I give up, plotly's defaults are shite and they haven't fixed them in years; tried with annotate but it won't go below the axis
    yaxis = list(title  = '')
  ) %>%
  add_annotations(             # because plotly still doesn't know how to rotate axes
    text = c('Height in cm'),
...
data.frame(x=letters[1:2], y = 1) %>% 
  ...
  scale_fill_manual(values = c('#F8766D', '#440154'))
...
K_ = Kfn(c(Xtrain, Xtest),
         l = l,
         sigmaf = sigmaf,
         sigman = sigman)                                                          # initial matrix
...
y2 = data.frame(t(mvtnorm::rmvnorm(
  npostpred, mean = postMu, sigma = postCov
)))
