...
<span class="emph">Layers</span> are added via piping. The first layers added are typically <span class="emph">geoms</span>, or geometric objects that represent the data, and include things like:
...
In case you're wondering why ggplot doesn't use `%>%` as in the tidyverse, it's because <span class="pack">ggplot2</span> was using pipes before it was cool, well before those came along (<span class="emph">+</span>). Otherwise, the concept is the same as before.
