...
Here is an example with the <span class="pack">nycflights</span> data, which regards flights that departed New York City in 2013.  The <span class="objclass">weather</span> data set has columns for things like temperature, humidity, and so forth.[^tibbledf]
...
```{r purr_example, eval=TRUE, echo=-1}
map = purrr::map  # could not remove conflict w
library(purrr)
x = list(1:3, 4:6, 7:9)
x %>% 
  map(sum)
...
Aside from data frames, you may think you don't have much need for list objects.  However, list objects make it very easy to iterate
...
Let's go further and create a plot of these results. We'll map to a data frame, use gather to melt it to two columns of model and value, then use ggplot2 to plot the results. Note that ggplot2 will change the ordering of the variable unless you coerce it by creating a factor[^ggplotalpha].
...
```{r vectorapp_ex1, echo=1, eval=FALSE}
x = matrix(1:9, 3, 3)
apply()
```
...
```{r vectorapp_ex2, echo=1, eval=FALSE}
x = list(1:3, 4:6, 7:9)
lapply()
sapply()
```
...
[^purrr]: Personally I find the help file description nebulous and examples unrealistic (group_by -> lm?), and oftentimes can come up with a group_by-summarise approach before I can sort out why the map approach didn't work.

[^ggplotalpha]: This pretty much defeats the entire purpose of retaining characters over factors everywhere else in the tidyverse.