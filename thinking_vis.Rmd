...
The following shows a plot of water risk for many countries. The first plots the color along a continuum, which is appropriate for this score of 0-5. We can clearly see problematic ones while still getting a sense of where other countries lie along that score.  The plot on the left arbitrarily codes a different color scheme, which might suggest some countries are fundamentally different than others.  However, if the goal is to show values relative to the median, then it accurately conveys countries above and below that value. If the median is not a useful value (e.g. to take some action upon), then the former plot would likely be preferred.

```{r valenced-plot, eval = FALSE}
...
p2 =  ggplot(wr, aes(map_id = name_0)) +
  ...
  scale_fill_gradient2(
    low = muted("#001260"), # based on vik extremes
    mid = "white",
    high = muted("#601200"),
...
The following uses the <span class="objclass">diamonds</span> data set that comes with <span class="pack">ggplot2</span>.  Use the <span class="func">scale_color_viridis</span> or <span class="func">scale_color_scico</span> function to add a more accessible palette.
