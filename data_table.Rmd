...
timing_join %>% kable_df(justify='lr')
# timing_join =
...
```{r dtsetkey, cache=FALSE, eval=FALSE}
...
```{r dt-group-timings, echo=FALSE, eval=FALSE}
timing_group_by_1 =
...
as.data.table(
  sapply(timing_group_by_big, `[[`, "elapsed"), keep.rownames = TRUE
  )[, .(fun = V1, elapsed = V2)][order(-elapsed)]
```
