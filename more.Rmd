...
A lot has been written about coding <span class="emph">style</span> over the decades.  If there was a definitive answer, you would have heard of it by now.  However, there are a couple things you can do at the beginning of your programming approach to go a long way making your code notably better.
...
You see some using `<-` or `=` for assignment.  While there is a slight difference, if you're writing decent code it shouldn't matter. Far more programming languages use `=`, so that's reason to prefer it.  However, if you like being snobby about things, go with `<-`.  Whichever you go with, use it consistently.
...
Don't be stingy with spaces. As you start out, err on the side of using them.  Just note there are exceptions (e.g. no space between function name and parenthsis, unless that function is something like <span class="func">if</span> or <span class="func">else</span>), but you'll get used to those over time.
...
Use tools like the build in RStudio code cleanup shortcut like `Ctrl/Cmd + Shft + A`. It's not perfect, in the sense I disagree with some of its the style choice, but it will be better than you will do on your own starting out.

#### Naming things

You might not think of it as such initially, but one of the more difficult challenges in programming is naming things.  Even if we can come up with a name, there are different styles we can use for the name.

Here is a brief list of things to keep in mind.

- Don't name something that is already an R function/object (e.g. `T`, <span class="func">c</span>, etc.)
- Try to be concise
- Make liberal use of suffixes/prefixes for naming the same types of things
...
For objects and functions, I find snake case easier[^style_claim] to read and less prone to issues. For example, camel case can fail miserably when acronyms are involved. Dots already have specific uses (file name extensions, function methods, etc.), so should be avoided unless you're using them for that specific purpose (they can also make selecting the whole name difficult depending on the context).
...
lm(hwy ~ cyl + displ + year, data=mpg)
```


[^ifelse]: For those familiar with <span class="func">ifelse</span>, that would be applicable, but is not the point of the example.

[^style_claim]: Note how I didn't say it's easier to read without qualification (even though I think it's obviously easier to read, based on how eyes/brains work).  Many style guides you encounter will try to come across as definitive.  They are not, nor are most claims tested.